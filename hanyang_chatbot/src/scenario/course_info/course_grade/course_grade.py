...
#         print(len(pre_info))
        ...
        course_grade = list(pre_info['학점'])
        course_grade = list(set(course_grade))
        
        result = ''
        for i in range(len(course_grade)):
            if i == (len(course_grade)-1):
                result += str(course_grade[i])
            else:
                result += str(course_grade[i]) + ', '
            

        
        return "\"" + course_name + "\" 과목은 " + result + " 학점짜리 수업입니다."
